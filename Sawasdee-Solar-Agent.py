...
วิธีใช้:
1. ไปที่ https://developers.line.biz -> เลือก OA สวัสดีโซลาร์
2. เปิด Messaging API -> เอา Channel Access Token มาใส่ใน LINE_TOKEN
3. เอาไฟล์นี้ไปวางบน Render.com / Railway.app ฟรี
4. เอา URL ที่ได้ไปใส่ใน Webhook URL ของ LINE Developers

ลูกค้าส่งรูปบิล + แชร์โลเคชั่น -> บอทตอบใบเสนอราคาอัตโนมัติ
"""
...
# ใส่ Token ของคุณตรงนี้ - ห้ามแชร์ให้คนอื่น
...
# หน่วยความจำชั่วคราว เก็บข้อมูลลูกค้าแต่ละคน
user_data = {}

def calculate_solar(bill_baht, elec_rate=4.7, sun_hours=4.5):
    """คำนวณขนาดระบบจากบิลค่าไฟ"""
    units_per_month = bill_baht / elec_rate
    # สูตร: ขนาด kW = หน่วยต่อวัน / ชั่วโมงแดด * 1.25 เผื่อ loss
    ...
    panels = math.ceil(system_kw * 1000 / 550)  # แผง 550W
    area = panels * 2.5  # ตรม.
    ...
    cost = system_kw * 32000  # ราคาติดตั้งต่อ kW
...
def build_flex_message(result, location_text=""):
    """สร้าง Flex Message สวยๆ ส่งกลับไปในไลน์"""
    ...
    flex_content = {
        ...
        "hero": {
            "type": "image",
            "url": "https://images.unsplash.com/photo-1509391366360-2e959784a276?w=800",  # รูปหลังคาโซลาร์
            "size": "full",
            "aspectRatio": "20:13",
            "aspectMode": "cover"
        },
...
def handle_image(event):
    # ในเวอร์ชั่นจริง ตรงนี้จะเรียก AI Vision อ่านบิล
    # ตอนนี้ทำแบบ mock ให้ตอบกลับไปขอตัวเลขก่อน
